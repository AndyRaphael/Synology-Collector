...
The storage, Active Backup, Hyper Backup, and Active Backup for Microsoft 365 /
Google Workspace APIs require an **administrators-group** account. A standard user
receives permission errors (DSM code 105) and the collector reports the affected
area as inaccessible. (Hyper Backup and the two SaaS backup products are optional
packages: if one is not installed, the collector reports it as `NOT_INSTALLED` —
an expected, healthy state, not an error.)
